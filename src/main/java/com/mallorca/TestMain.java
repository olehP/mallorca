...
public class TestMain {
	...
	// private static final double PERFECT_RATIO = ((double) IMG_WIDTH) /
	// IMG_HEIGHT;
	//
	// public static void main(String[] args) throws IOException {
	// createImage("visit rock of gibraltar",
	// "D:\\work\\projects\\mallorca\\rock.jpg", "rsult.png");
	// }
	//
	// private static void createImage(String text, String imageUrl, String
	// resultImagePath) throws IOException {
	// if (text.length() > 40) {
	// throw new RuntimeException("Sorry, but text is too long(max 40 symbols
	// are allowed)");
	// }
	// if (text.length() < 28) {
	...
	// int index = 1;
	// while (index < splited.length && firstLine.length() +
	// splited[index].length() < 27) {
	// firstLine.append(" ");
	...
	// throw new RuntimeException("Sorry but text has wrong format, please try
	// another");
	// }
	// addTwoLineText(firstLine.toString(), secondLine.toString(), imageUrl,
	// resultImagePath);
	// }
	//
	// }
	//
	// private static void addTwoLineText(String firstLine, String secondLine,
	// String imageUrl, String resultImagePath)
	// throws IOException {
	// BufferedImage first = addShadow(firstLine);
	// BufferedImage second = addShadow(secondLine);
	// File file = new File(imageUrl);
	// BufferedImage source = ImageIO.read(file);
	// int type = source.getType() == 0 ? BufferedImage.TYPE_INT_ARGB :
	// source.getType();
	// source = cropImageToStandartSize(source);
	// source = resizeImage(source, type, IMG_WIDTH, IMG_HEIGHT);
	// Graphics g = source.getGraphics();
	// g.drawImage(first, IMG_WIDTH / 2 - first.getWidth() / 2 + 7, IMG_HEIGHT /
	// 2 - 55, null);
	// g.drawImage(second, IMG_WIDTH / 2 - second.getWidth() / 2 + 7, IMG_HEIGHT
	// / 2 - 10, null);
	...
	// return image.getSubimage(0, (image.getHeight() - (int) (image.getWidth()
	// / PERFECT_RATIO)) / 2, image.getWidth(),
	// (int) (image.getWidth() / PERFECT_RATIO));
	// }
	//
	// return image.getSubimage((image.getWidth() - (int) (image.getHeight() *
	// PERFECT_RATIO)) / 2, 0, (int)
	// (image.getHeight() * PERFECT_RATIO), image.getHeight());
	//
	// }
	//
	// private static void addOneLineText(String text, String imageUrl, String
	// resultImagePath) throws IOException {
	...
	// source = resizeImage(source, type, IMG_WIDTH, IMG_HEIGHT);
	// Graphics g = source.getGraphics();
	// g.drawImage(image, source.getWidth() / 2 - image.getWidth() / 2 + 7,
	// source.getHeight() / 2 - image.getHeight() /
	// 2, null);
	// ImageIO.write(source, "png", new File(resultImagePath));
	// }
	//
	// private static BufferedImage addShadow(String text) throws IOException {
	// BufferedImage image = convertTextToGraphic(text, new Font("Arial",
	// Font.PLAIN, 45));
	...
	// }
	//
	// public static BufferedImage generateBlur(BufferedImage imgSource, int
	// size, Color color, float alpha) {
	...
	// }
	//
	// public static BufferedImage applyShadow(BufferedImage imgSource, int
	// size, Color color, float alpha) throws
	// IOException {
	...
	// }
	//
	// public static BufferedImage generateShadow(BufferedImage imgSource, int
	// size, Color color, float alpha) {
	...
	// imgGlow = resizeImage(imgGlow, 2, imgGlow.getWidth() + size * 2,
	// imgGlow.getHeight() + size * 2);
	// return imgGlow;
	// }
	//
	// public static Image applyMask(BufferedImage sourceImage, BufferedImage
	// maskImage) {
