...
public interface MomentDAO extends JpaRepository<Moment, Integer> {
	@Query(value = "SELECT m.* FROM moment as m, user_moment as us WHERE (us.moment_id != m.id OR us.user_id != :userId) AND  (m.text  LIKE :first OR m.text LIKE :second) LIMIT :page, :offset", nativeQuery = true)
	...
	@Query(value = "SELECT m.* FROM moment as m, user_moment as us WHERE us.moment_id != m.id OR us.user_id != :userId LIMIT :page, :offset", nativeQuery = true)
	public List<Moment> searchWithoutQuery(@Param("page") Integer page, @Param("offset") Integer offset,  @Param("userId") Integer userId);

	@Query("SELECT um.moment FROM  UserMoment um WHERE um.user = :user AND um.status = :state")
